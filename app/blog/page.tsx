...
export const metadata: Metadata = {
  ...
  description: 'Articles on data science, Python, machine learning, and teaching tech by George Edwin.',
};
...
const posts = [
  {
    ...
    excerpt: 'How to serve a trained ML model in production using FastAPI and Redis Streams — with retries, queuing, and monitoring baked in.',
  ...
  },
  {
    ...
    excerpt: 'A relational mindset is the foundation of good data thinking. Here is why SQL first makes you a better engineer, full-stop.',
  ...
  },
  {
    ...
    excerpt: 'Reflections on delivering competency-based Python lessons to diploma students — what worked, what didn\'t, and the tools that made the difference.',
  ...
  },
  {
    ...
    excerpt: 'A step-by-step walkthrough of Exploratory Data Analysis using Pandas — from raw CSV to actionable insights, with real Kenyan datasets.',
        ...
        <div className="space-y-4">
          {posts.map(p => (
            <Link
              key={p.slug}
              href={`/blog/${p.slug}`}
              className="block terminal-card no-underline transition-all"
            ...
            >
              <div className="p-5">
                ...
                <h2 className="font-mono font-bold text-base mb-2" style={{ color: 'var(--text)' }}>
                  {p.title}
                </h2>
                <p className="font-mono text-xs leading-relaxed" style={{ color: 'var(--muted)' }}>
                  {p.excerpt}
                </p>
                <p className="font-mono text-xs mt-3" style={{ color: 'var(--green)' }}>
                  → Read more
                </p>
              </div>
            </Link>
